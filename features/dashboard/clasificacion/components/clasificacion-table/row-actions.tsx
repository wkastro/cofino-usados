...
import { deleteClasificacion, toggleEstadoClasificacion } from "../../actions/clasificacion.actions"
import type { Clasificacion, ClasificacionTipo } from "../../types/clasificacion"

interface RowActionsProps {
  row: Clasificacion
  tipo: ClasificacionTipo
  onEdit: (row: Clasificacion) => void
}
...
export function RowActions({ row, tipo, onEdit }: RowActionsProps) {
  ...
  function handleToggle() {
    startTransition(async () => {
      const result = await toggleEstadoClasificacion(tipo, row.id, !row.estado)
  ...
  function handleConfirmDelete() {
    startTransition(async () => {
      const result = await deleteClasificacion(tipo, row.id)
      if (result.ok) toast.success(result.message)
      else toast.error(result.message)
      setDeleteOpen(false)
    })
